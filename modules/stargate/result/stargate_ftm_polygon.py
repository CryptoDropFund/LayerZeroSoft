...
# Fantom -> Polygon USDC Bridge


def swap_usdc_fantom_to_polygon(amount, PRIVATE_KEY):
    ...
    fees = stargate_fantom_contract.functions.quoteLayerZeroFee(POLYGON_ID,
                                                                TYPE_SWAP_REMOTE,
    ...
    try:
        ...
        if receipt['status'] != 1:
            print(
                f"[STARGATE] Can't send transaction Fantom -> Polygon")
            return -1
        print(
            f'[STARGATE] Success transfer USDC for {decimal_to_int(amount, 6)} from Fantom to Polygon')
        return 1
    except:
        print(f'[STARGATE] Exception catched. Retry')
        return -1
...
def swap_usdc_polygon_to_fantom(amount, PRIVATE_KEY):
    ...
    min_amount = amount - (amount * SLIPPAGE) // 1000
    fees = stargate_polygon_contract.functions.quoteLayerZeroFee(FANTOM_ID,
                                                                 TYPE_SWAP_REMOTE,
                                                                 "0x0000000000000000000000000000000000000001",
                                                                 PAYLOAD_VALUE,
                                                                 EXTRA_GAS_PARAMS
    ...
    if allowance < amount:
        check_enough_gas(polygon_w3, address, GAS_LIMIT)
        try:
            ...
            if receipt['status'] != 1:
                ...
                return -1
            print(
                f"[STARGATE] POLYGON | USDC APPROVED | https://snowtrace.io/tx/{approve_txn_hash.hex()} ")
            nonce += 1
        except:
            print(f'[STARGATE] Exception catched. Retry')
            return -1
    ...
    try:
        ...
        if receipt['status'] != 1:
            print(
                f"[STARGATE] Can't send transaction Polygon -> Fantom")
            return -1
        print(
            f'[STARGATE] Success transfer USDC for {decimal_to_int(amount, 6)} from Polygon to Fantom')
        return -1
    except:
        print(f'[STARGATE] Exception catched. Retry')
        return -1
